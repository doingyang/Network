...
import com.library.retrofit.manager.HttpManager;
 ...
 *
 * @see HttpManager 这里封装了通用的方法，请求的参数都在这里封装。
 * <p>
...
public interface CommonRequest {
    ...
    @DELETE("{path}")
    Observable<ResponseBody> doDelete(
            @Path(value = "path", encoded = true) String url
    );
    ...
    @GET
    Observable<ResponseBody> doGetFullPath(
            @Url String url,
            @QueryMap Map<String, Object> map
    );
    ...
    @FormUrlEncoded
    @POST
    Observable<ResponseBody> doPostFullPath(
            @Url String url,
            @FieldMap Map<String, Object> map
    );
    ...
    @Multipart
    @POST
    Observable<ResponseBody> uploadFileFullPath(
            @Url String url,
            @Part("description") RequestBody description,
            @Part MultipartBody.Part file
    );
